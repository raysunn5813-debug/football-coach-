...
def simulate_snap(play_key: str):
    ...
    hb = {"role": "HB", "state": "PRE_SNAP", "route_waypoints": [{"x": 30, "z": 26}]}
    ...
    if is_run:
        ...
        ball["x"], ball["z"] = hb["x"], hb["z"]  # type: ignore
    ...
    return {
        ...
        "wlb_state": wlb["state"],
        "bsg_state": bsg["state"],
        "ball_on_hb_at_snap": ball.get("x") == getattr(hb, "x", hb.get("x")),
    }


print("=== SNAP simulation (frontend sends play KEY) ===")
runs = [p for p in PLAY_DATABASE if p.get("play_type") == "RUN"][:5]
passes = [p for p in PLAY_DATABASE if p.get("play_type") != "RUN"][:3]

for p in runs + passes:
    key = p.get("key", p.get("id"))
    r = simulate_snap(key)
    if r:
        ...
        print(f"[{match}] key={key!r} schema={schema} detected={detected} | QB={r['qb_state']} HB={r['hb_state']} WLB={r['wlb_state']} BSG={r['bsg_state']}")

print()
print("=== If play NAME were sent (report scenario only) ===")
for name in ["I-Form Power 17", "Pistol Inside Zone", "36 Power", "27 Stretch Wide Zone"]:
    p = next((x for x in PLAY_DATABASE if x.get("name") == name), None)
    if not p:
        continue
    detected = backend_is_run_play(name)
    schema = p.get("play_type") == "RUN"
